...
df %>%
  ...
  ggplot(aes(x=year,fill=disastertype))+geom_bar()+labs(
    title="Nature Disaster in Beijing, 1980-now")
...
```{r}
df %>%
  filter(adm1=="Heilongjiang"|adm1=="Jilin"|adm1=="Liaoning") %>%
  #group_by(year,disastertype) %>%
  ggplot(aes(x=disastertype,fill=disastertype))+geom_bar()+labs(title="Nature Diseaster type in Northeast")
```
